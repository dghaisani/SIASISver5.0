...
public class PollingController extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageView back = (ImageView) findViewById(R.id.back_createpolling);
        ...
        sendPolling.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sJudul.equals("") || sJudul.trim().length() <= 0){
                    judulPolling.setHint("Judul tidak boleh kosong");
                    judulPolling.setHintTextColor(Color.parseColor("#FF0000"));
                    temp = false;
                } else if (sJudul.length() > 25) {
                    judulPolling.setText(null);
                    judulPolling.setHintTextColor(Color.parseColor("#FF0000"));
                    judulPolling.setHint("Judul, max 25 karakter");
                    temp = false;
                }

                if (sDeskripsi.equals("") || sDeskripsi.trim().length() <= 0) {
                    deskripsiPolling.setHintTextColor(Color.parseColor("#FF0000"));
                    temp = false;
                } else if (sDeskripsi.length() > 255) {
                    deskripsiPolling.setText(null);
                    deskripsiPolling.setHintTextColor(Color.parseColor("#FF0000"));
                    deskripsiPolling.setHint("contoh: 2403, max 10 karakter");
                    temp = false;
                }
